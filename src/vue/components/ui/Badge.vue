...
const variantClasses: Record<BadgeVariant, { bg: string; dot: string }> = {
  default: { bg: 'bg-[#151B28] text-[#94A3B8] border border-[#1E293B]', dot: 'bg-[#94A3B8]' },
  ...
  info: { bg: 'bg-[#60A5FA]/10 text-[#60A5FA] border border-[#60A5FA]/30', dot: 'bg-[#60A5FA]' },
  ...
  neutral: { bg: 'bg-[#0B0E14] text-[#64748B] border border-[#1E293B]', dot: 'bg-[#64748B]' },
  outline: { bg: 'bg-transparent text-[#94A3B8] border border-[#1E293B]', dot: 'bg-[#94A3B8]' },
};

const classes = computed(() => {
  const v = variantClasses[props.variant];
  return `inline-flex items-center font-medium rounded ${props.mono ? 'font-mono' : 'font-sans'} ${sizeClasses[props.size]} ${v.bg}`;
});
